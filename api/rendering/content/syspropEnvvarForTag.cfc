...
component {
	function render( required any docTree, required string contentType, required struct args, boolean markdown=false ) {
		...
		// Determine tag name from page ID (e.g., "tag-query" -> "query")
		var pageId = page.getId();
		if ( left( pageId, 4 ) != "tag-" ) {
			return ""; // Not a tag page
		}

		var tagName = mid( pageId, 5 );

		// Get related sysprops
		var sysprops = arguments.docTree.getSyspropsByTag( tagName );
