	...
	}'
{
	...
	public function getFileCommitDates(required string repoPath, required string folder, required struct files, required string mode) {
		try {
			...
			if (!DirectoryExists(gitPath))	throw(".git not found at [#gitPath#]");
			var repository = new FileRepositoryBuilder().setGitDir(
				new File(gitPath)).readEnvironment().findGitDir().build();
	...
	private function getAllDatesforFile( repository, filePath ){
		...
		while (commitIter.hasNext()) {
			...
			commits.append(
				createObject("java", "java.util.Date").init(commit.getCommitTime() * 1000)
	...
	private function getCreatedUpdatedForFile( repository, filePath ){
		...
		if (!isNull(lastModifiedCommit)) {
			commits.append( createObject("java", "java.util.Date").init(lastModifiedCommit.getCommitTime() * 1000) );
		}
		revWalk.reset();
		revWalk.markStart(headCommit);
		...
		if (!isNull(createdCommit)) {
			commits.append( createObject("java", "java.util.Date").init(createdCommit.getCommitTime() * 1000) );
		}
