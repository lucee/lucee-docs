...
// LUCEEDOCS SPECIFIC CODE
( function(){
  var anchors    = new AnchorJS()
    , $fixedMenu = $( "header.header" )
    , scrollBeneathFixedMenu;
  ...
  scrollBeneathFixedMenu = function() {
    var $target = $( window.location.hash )
      , fixedMenuOffset, targetTop;

    if ( $target.length  ) {
      fixedMenuOffset = $fixedMenu.height() + 20;
      targetTop       = $target.offset().top;
      $('html, body').scrollTop( targetTop - fixedMenuOffset );
    }
  ...
  $( '.anchorjs-link' ).click( function( e ){
    e.preventDefault();
    window.location.hash = $(this).get(0).hash;
    scrollBeneathFixedMenu();
  } );
