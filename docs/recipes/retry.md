...
The `retry` statement jumps back to the beginning of a try block - useful for recovering from errors and retrying an operation.
...
This is not the best way. `retry` is a better option:
...
In this example, we use `retry`. Here we also still check if the file exists or not. If the file does not exist, we create a new file by using `fileWrite`. Then call `retry` to avoid duplicate code. `retry` points to the beginning of the try block and then it will read again the file and output the file content.
