...
Secrets return a lazy reference — the actual value is only resolved when used.

This means you can safely store references at application startup and they will always reflect the current secret value, even after rotation:
...
For local development you can mock AWS Secrets Manager and Parameter Store using [LocalStack](https://localstack.cloud/).

Configure the `endpoint` property to point to your LocalStack instance instead of real AWS:
