**Deprecated** — use the [[function-querylazy]] function instead, which provides explicit row/block streaming via a listener callback and a bounded connection lifecycle.
...
When iteration completes the underlying ResultSet is held until the request ends or [[function-queryclose]] is called explicitly. This is the main reason `queryLazy()` is preferred — its connection lifetime is bounded by the function call.
