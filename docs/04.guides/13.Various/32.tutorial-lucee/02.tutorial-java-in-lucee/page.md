...
Having said that, there are some instances where it makes sense to use Java objects directly: use
...
Step 1 - check out the API of the objects you plan to use -- you have to know which methods and properties are available for you from your application.

Step 2 - get a reference to Java object(s) in CFML via the createObject( "java", objectType ) function.
...
the way to fix this is to force CF to pass the argument as an Integer. you will do that like so:
